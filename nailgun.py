...
def create_environment(fuel_ip, kvm_count, machines_count, cluster_settings):
    ...
    if 'repo_setup' in attributes['editable']:
        custom_repos_list = json.loads(cluster_settings.get('repo_setup', []))
        if len(custom_repos_list):
            for current_repo in custom_repos_list:
                attributes['editable']['repo_setup']['repos']['value'].append(current_repo)
